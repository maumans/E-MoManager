...
function Index({auth,success,errors,locations}) {
    ...
    function handleClick() {
        Inertia.get(route("gestion.location.create",[auth.user.id]),{preserveScroll:true})
    }
                        ...
                        <motion.div
                            ...
                            <div className={'space-y-5 p-5 '}>
                                ...
                                <div className={"break-words flex justify-between gap-4 flex-wrap"}>
                                    ...
                                    <div>
                                        <span className={"p-2 text-white rounded font-bold appThemeBackgroundColor"}>{formatNumber(location?.loyerTotal)} FG</span>
                                    </div>
                                </div>

                                <div className={"flex gap-5"}>
                                    <button onClick={handleClick} className="bg-green-500 hover:bg-green-600 text-white h-full py-2 px-4 rounded">
                                        Revenu <AddCircleOutlineIcon/>
                                    </button>

                                    <button onClick={handleClick} className="bg-red-500 hover:bg-red-600 text-white h-full py-2 px-4 rounded">
