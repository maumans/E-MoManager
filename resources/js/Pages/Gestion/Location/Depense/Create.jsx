...
function Create({auth,errors,location,revenus,societe,loyers,devises,success}) {
    ...
    /*useEffect(()=>{
        console.log(location.devise.symbole)
        setData("devise",location.devise.code)
    },[])*/
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route("gestion.location.depense.store",[auth.user.id,location.id]),{preserveScroll:true})
    }
                            ...
                            <div className={"w-full flex gap-2"}>
                                <div className={"flex-grow"}>
                                    <TextField
                                        ...
                                        className={"w-full"} label="Montant" name="montant" onChange={onHandleChange}/>
                                   {/* <Select
                                        className={"bg-white w-4/12"}
                                        value={data.devise}
                                        name="devise"
                                ...
                                <div className={"w-fit"}>
                                    <Autocomplete
                                        value={data.devise}
                                        onChange={(e,val)=>setData("devise",val)}
                                        disablePortal={true}
                                        options={devises}
                                        getOptionLabel={(option)=>option.symbole}
                                        isOptionEqualToValue={(option, value) => option.id === value.id}
                                        renderInput={(params)=><TextField  fullWidth {...params} placeholder={"devise"} label={params.symbole}/>}
                                        required
                                    />
                                    <div className={"text-red-600"}>{errors?.devise}</div>
                                </div>
                            </div>
                        </div>
                        <div className={"col-span-2 mt-8"}>
                            <TextareaAutosize className={"w-full"} name={"motif"} placeholder={"Motif"} style={{height:100}} onChange={onHandleChange} autoComplete="commentaire"/>
